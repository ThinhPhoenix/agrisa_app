...
export const NotificationBadge: React.FC<NotificationBadgeProps> = ({
    ...
    const { data, isLoading } = useMyNoti({ page: 1, limit: 10 });

    // API response structure: { data: [...], unread: number, pagination: {...} }
    const unreadCount = (data as any)?.unread ?? 0;

    // Sử dụng override count hoặc API unread count
    const count = overrideCount ?? unreadCount;

    // Hiển thị loading state hoặc không hiển thị nếu count = 0
    if (isLoading) {
        return null; // Có thể hiển thị loading indicator nếu cần
    }
