...
import { PublicBasePolicyResponse, RegisterPolicyPayload } from "../models/policy.models";
import { DocumentTagsSchema, formatDocumentTagsForSubmit } from "../utils/document-tags.utils";
...
export const usePolicyForm = ({ 
  ...
  const [documentTagsData, setDocumentTagsData] = useState<Record<string, any>>({});
  ...
  const isFormValid = useMemo(() => {
    ...
    if (documentTags && Object.keys(documentTags).length > 0) {
      const allFieldsFilled = Object.keys(documentTags).every((fieldName) => {
