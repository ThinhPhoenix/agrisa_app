...
export const usePolicy = () => {
    ...
    const useGetCancelRequestByPolicyId = (registered_policy_id: string) => {
        return useQuery({
            queryKey: [
                QueryKey.POLICY.GET_CANCEL_REQUESTS,
            ...
            queryFn: async () => {
                ...
                if (response.success && response.data) {
                    ...
                    console.log(
                        "🔍 Looking for policy_id:",
                        registered_policy_id
                    );

                    // Lọc cancel request theo registered_policy_id
                    const filteredClaim = response.data.claims?.find(
                        (claim) => {
                            console.log(
    ...
    const useGetRegisteredPolicyDetail = (
        ...
        return useQuery({
            ...
            queryFn: () =>
                policyServices.get.get_registered_policy_detail(policy_id),
            enabled:
                options?.enabled !== undefined ? options.enabled : !!policy_id,
        });
    };
        ...
        onError: (error: any) => {
            ...
            if (errorCode === "INVALID_REQUEST") {
                ...
                if (apiMessage.toLowerCase().includes("base_policy_id")) {
                    ...
                    apiMessage.toLowerCase().includes("coverage_amount")
                ) {
                    ...
                    apiMessage.toLowerCase().includes("area_multiplier")
                ) {
                    ...
                    errorMessage =
                        "Vui lòng cung cấp thông tin vị trí trang trại.";
                ...
                if (
                    ...
                    apiMessage
                        .toLowerCase()
                        .includes("enrollment dates are required")
                ) {
                    errorMessage =
                    ...
                    apiMessage
                        .toLowerCase()
                        .includes("base policy is not active") ||
    ...
    const cancelPolicyMutation = useMutation({
        ...
        onError: (error: any) => {
            ...
            if (apiMessage.toLowerCase().includes("policy not found")) {
                ...
                apiMessage.toLowerCase().includes("pending cancel request")
            ) {
