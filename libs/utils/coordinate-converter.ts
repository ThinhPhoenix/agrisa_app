import { convertVn2000ToWgs84Api } from "@/domains/shared/services/vn2000-api.service";
...
const VN2000_3TM_105 =
  "+proj=tmerc +lat_0=0 +lon_0=105 +k=0.9999 +x_0=500000 +y_0=0 +ellps=WGS84 +units=m +no_defs"; // EPSG:9212 - Zone 3, 105°E (Hà Nội, Bắc Bộ)
const VN2000_3TM_106 =
  "+proj=tmerc +lat_0=0 +lon_0=106 +k=0.9999 +x_0=500000 +y_0=0 +ellps=WGS84 +units=m +no_defs"; // Múi 106°E (Thanh Hoá, Nghệ An)
const VN2000_3TM_107 =
  "+proj=tmerc +lat_0=0 +lon_0=107 +k=0.9999 +x_0=500000 +y_0=0 +ellps=WGS84 +units=m +no_defs"; // EPSG:9213 - Zone 4, 107°E (Huế, Đà Nẵng)
const VN2000_3TM_108 =
  "+proj=tmerc +lat_0=0 +lon_0=108 +k=0.9999 +x_0=500000 +y_0=0 +ellps=WGS84 +units=m +no_defs"; // EPSG:9214 - Zone 5, 108°E (Lâm Đồng, Khánh Hòa)
const VN2000_3TM_109 =
  "+proj=tmerc +lat_0=0 +lon_0=109 +k=0.9999 +x_0=500000 +y_0=0 +ellps=WGS84 +units=m +no_defs"; // EPSG:9215 - Zone 6, 109°E (Đồng Nai, Bình Dương)
...
const PROVINCE_TO_ZONE: Record<string, number> = {
  ...
  "Gia Lai": 5,
  "Đắc Lắc": 5,
  "Lâm Đồng": 5, // ⭐ Lâm Đồng dùng 108°E
...
function getVN2000ZoneProjection(zone?: number, province?: string): string {
  ...
  if (province) {
    ...
    if (detectedZone && ZONE_TO_PROJECTION[detectedZone]) {
      console.log(
        `🗺️ Auto-detected zone ${detectedZone} from province: ${province}`
      );
...
export class CoordinateConverter {
    ...
    try {
      ...
      const convertedCoords = await Promise.all(
        boundary.coordinates[0].map(async ([x, y]) => {
          ...
          const { latitude, longitude } = await this.vn2000ToWgs84(
            x,
            y,
            province
          );
  ...
  static isInVietnam(latitude: number, longitude: number): boolean {
    ...
    return (
      latitude >= 8 && latitude <= 24 && longitude >= 102 && longitude <= 110
    );
  }
